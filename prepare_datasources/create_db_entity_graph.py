...
from paths import DB_ROOT, TIMESTAMP_EPISODES_ROOT
...
def build_db_videomme(json_folder: str, dataset: str, config: str) -> None:
    ...
    for jf in json_files:
        ...
        db_path = DB_ROOT / f"entity_graph/{dataset}/{config}/videomme_{video_id}.db"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build SQLite entity_graph_table from entity graph JSONs produced by create_entity_graph.py."
    )
    parser.add_argument(
        "--dataset",
        type=str,
        ...
        help="Dataset name (default: egolife).",
    )
    parser.add_argument(
        "--config",
        type=str,
        default="fused_dt_and_gpt-4.1_captions",
        help="Config folder name under TIMESTAMP_EPISODES_ROOT, e.g. fused_dt_and_gpt-4.1_captions, fused_dt_and_llava-video-7bcaptions (default: fused_dt_and_gpt-4.1_captions).",
    )
    ...
    db_path = DB_ROOT / f"entity_graph/{dataset}_entity_graph_{config}.db"
